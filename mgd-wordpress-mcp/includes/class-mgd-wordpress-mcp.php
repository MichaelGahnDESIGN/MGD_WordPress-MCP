...
final class MGD_WordPress_MCP {
    ...
    public static function defaults() {
        return array( 'writes_enabled' => false, 'maintenance_enabled' => false, 'divi_writes_enabled' => false, 'media_imports_enabled' => false, 'github_updates' => true, 'audit_enabled' => true, 'max_upload_mb' => 8 );
    }
    ...
    public function dependency_notice() {
        ...
        if ( ! function_exists( 'wp_register_ability' ) ) {
            echo '<div class="notice notice-error"><p><strong>MGD WordPress MCP:</strong> ' . esc_html__( 'Die WordPress Abilities API ist nicht verfügbar. Bitte WordPress 6.9 oder neuer verwenden.', 'mgd-wordpress-mcp' ) . '</p></div>';
            return;
        }
        if ( ! class_exists( '\\WP\\MCP\\Core\\McpAdapter' ) ) {
            echo '<div class="notice notice-warning"><p><strong>MGD WordPress MCP:</strong> ' . esc_html__( 'Für MCP-Verbindungen wird der offizielle WordPress MCP Adapter benötigt.', 'mgd-wordpress-mcp' ) . ' <a href="' . esc_url( 'https://github.com/WordPress/mcp-adapter/releases/latest' ) . '" target="_blank" rel="noopener">' . esc_html__( 'MCP Adapter herunterladen', 'mgd-wordpress-mcp' ) . '</a></p></div>';
        }
    }
    ...
    public function register_custom_mcp_server( $adapter ) {
        ...
        try {
            $result = $adapter->create_server(
                ...
                'WordPress management tools by Michael Gahn DESIGN.', MGD_WPMCP_VERSION,
                array( \WP\MCP\Transport\HttpTransport::class ),
                \WP\MCP\Infrastructure\ErrorHandling\ErrorLogMcpErrorHandler::class,
                \WP\MCP\Infrastructure\Observability\NullMcpObservabilityHandler::class,
                $tools, array(), array(),
                static function () { return is_user_logged_in() && current_user_can( 'read' ); }
            );
            if ( is_wp_error( $result ) && defined( 'WP_DEBUG' ) && WP_DEBUG ) { error_log( 'MGD WordPress MCP server registration failed: ' . $result->get_error_message() ); }
        } catch ( Throwable $e ) {
            if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) { error_log( 'MGD WordPress MCP server registration failed: ' . $e->getMessage() ); }
        }
    }
